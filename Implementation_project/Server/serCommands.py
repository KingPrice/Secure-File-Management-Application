...
def login(conn, addr):
    while True:
        ...
        try:
            ...
            logging.info(cn + " Has logged in")
            conn.send("Valid".encode())
            break
        except:
            logging.info(f"invalid username or password from {addr}")
            conn.send('1'.encode())
    cn = 'cn=' + cn
...
def write(conn, perms):
    # Turns the received message to ascii string
    msg = conn.recv(1024).decode("utf-8")
    print(msg)
    logging.info(perms + " has sent " + msg)
    conn.close()
    return
    ...
    if perms == "1002" or perms == "1001":
        ...
        with open('files/' + filename, "rb") as f:
            while True:
                ...
                progress.update(len(bytes_read))
        logging.info(perms + "has downloaded " + filename)
        conn.close()
    else:
        conn.send("Permission Denied".encode())
...
def delete(conn, perms):
    ...
    if perms == '1002':
        ...
        logging.info(perms + " has deleted " + filename)
    else:
        conn.send("Permission Denied".encode())
        print("Connection closed")
        logging.info(perms + " attempted to use Delete")
